...
- Admin closes registration (`POST /tournament/admin/close-registration`, header `X-Admin-Key: <TOURNAMENT_ADMIN_KEY>`) once entries are done - this locks the entrant list, randomly splits entrants into 4 groups, and runs the full round-robin qualifying stage in the same request (every match a seeded coin-flip - the duel engine itself is "100% random, no stats," so qualifying honors that same equal-odds philosophy). `GET /tournament/groups`, `GET /tournament/groups/:num/matches` and `GET /tournament/top16` expose the results.
- Once qualifying is done, admin calls `POST /tournament/admin/advance-knockout` repeatedly to run the Top 16 knockout bracket - Round of 16 → Quarterfinals → Semifinals → 3rd Place Match → Final. Each call finishes whichever match is currently live (if any) and immediately decides + starts the next one; the very first call seeds Round of 16 from the Top 16 (standard bracket seeding - cosmetic only, combat stays 100% random). Every match's result is computed the instant it goes live, via `simulateKnockoutMatch` (`tournament-sim.js`) - a seeded, headless replay of tcg.html's entire HP duel engine that produces a full round-by-round log, not just a winner. `GET /tournament/bracket` returns every match (including that log once a match has gone live), which is what lets every visitor's client replay the identical fight instead of animating a fresh random one per browser - "played and shown live" means the same result for everyone.
- `POST /tournament/admin/reset` wipes entrants, qualifying results and the bracket, and reopens registration - the only way to start a new tournament once one has run.
...
- `advance-knockout` returns 400 "Need 16 qualified entrants": fewer than 16 units made it out of qualifying (e.g. a small test run) - the knockout stage needs a full Top 16 to seed Round of 16.
